...
int main(){
	...
	// Welcome message
	printf("\n*** Stack data structure implimented using doubly linked list ***\n\n");
	printf("When prompted, enter a one-letter command.\n");
	printf("'+' to push to the stack\n");
	printf("'-' to pop node item from the stack\n");
	printf("'p' to print the current stack\n");
	...
	while(1) {	//<= input loop
		...
		while (((in = getc(stdin)) != '\n') && (in != EOF)) {	//<= filter 'enter' character
			...
			case '+':
				...
				list_push(list, (void*)strtol(payload, &endptr, 10));
				break;
